<?php
if (($_GET['type'] == "switch") && ($_GET['token'] == "doubleq")) {//可以改token,这相当于密码，在Arduino端改成相应的值即可
...
}else if (($_GET['type'] == "sensor") && $_GET['data'] && ($_GET['token'] == "doubleq")){//可以改token,这相当于密码，在Arduino端改成相应的值即可
	$data = $_GET['data'];

	$con = mysql_connect(SAE_MYSQL_HOST_M.':'.SAE_MYSQL_PORT,SAE_MYSQL_USER,SAE_MYSQL_PASS);//进入数据库
